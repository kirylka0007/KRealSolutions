...
### Task 12: Privacy policy page and Companies Act trading disclosure

**Context:** the site collects personal data via two forms (the enquiry form and the new health-check quiz) and, as a UK limited company, has legal disclosure obligations this plan hasn't covered yet: a GDPR-compliant privacy notice, and the Companies Act trading disclosures (registered company name, number, and registered office address) that should appear on the website. This task is user-requested, added after the original 11-task plan and spec were approved — it does not appear in `docs/superpowers/specs/2026-08-17-site-restructure-design.md`, but is a small, self-contained addition that doesn't touch anything else in that spec.

**Company details confirmed by the user:** registered name "K Real Solutions Ltd", company number `SC891005`, registered in Scotland (already stated in the footer). Registered office address was not provided — leave it as a clearly-flagged placeholder (same convention already used in this codebase for the placeholder domain/email — see `layout.tsx`'s `PLACEHOLDER` comments), not a fabricated address. Do not register with the ICO or reference an ICO registration number as part of this task — the user is handling that separately.
...
**Interfaces:** none — a new static page plus small additions to two already-existing files.
...
- [ ] **Step 2: Add the Companies Act trading disclosure and a privacy link to `Footer.tsx`**
...
- [ ] **Step 3: Add `/privacy` to `sitemap.ts`**

Add one more entry to the array Task 1 built:
...
Grep the diff for `[registered office address` to confirm both placeholder instances (privacy page and footer) are present and clearly marked — this is a real legal-disclosure gap being deliberately deferred pending the user supplying the address, not an oversight; do not fabricate an address to fill it.
